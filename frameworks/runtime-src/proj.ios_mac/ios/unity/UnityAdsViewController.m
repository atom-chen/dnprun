...
//    [[UnityAds sharedInstance] startWithGameId:@"1070802" andViewController:[UnityAdManager getInstance].viewController];
...
- (void)openAds {
    ...
    if ([[UnityAds sharedInstance] setZone:@"video"] && [[UnityAds sharedInstance]canShow]) {
        ...
        [[UnityAds sharedInstance] show]; //Show an ad
         ...
         [[UnityAdManager getInstance] callbackLua:@"none"];
    }
...
- (void)hasAds {
    ...
    if ([[UnityAds sharedInstance] setZone:@"video"] && [[UnityAds sharedInstance]canShow]) {
        [[UnityAdManager getInstance] callbackLua1:@"ready"];
    }else{
         [[UnityAdManager getInstance] callbackLua1:@"none"];
    }
    NSLog(@"canShow: %i",[[UnityAds sharedInstance] canShow]);
}
...
- (void)unityAdsVideoCompleted:(NSString *)rewardItemKey skipped:(BOOL)skipped {
    ...
    [[UnityAdManager getInstance] callbackLua:@"complete"];
}
